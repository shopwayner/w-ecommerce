...
export type BlingFullProductModuleResult = {
  ...
  status: "NOT_REQUESTED" | "PENDING" | "COMPLETED" | "FAILED" | "VERIFICATION_FAILED";
};
...
function moduleResults(): BlingFullProductModuleResult[] {
  return BLING_FULL_PRODUCT_SYNC_MODULES.map((module) => ({ module, status: "PENDING" }));
}
...
async function preparePlan(
  ...
  const depositId = context.local.stock === null
    ? null
  ...
  const variations = Array.isArray(remote.variacoes) ? remote.variacoes : [];
  const structure = record(remote.estrutura);
  if ((format && format !== "S") || variations.length > 0 || Object.keys(structure).length > 0) {
...
export class BlingFullProductSyncService {
  ...
  async preview(input: {
    ...
    if (!Object.keys(plan.mainPayload).length) setModule(modules, "PRODUCT_FIELDS", "NOT_REQUESTED");
    if (!plan.priceCostPayload) setModule(modules, "PRICE_COST", "NOT_REQUESTED");
    if (!plan.stockPayload) {
      setModule(
        modules,
        "STOCK",
        plan.blockers.some((item) => item.includes("deposito")) ? "FAILED" : "NOT_REQUESTED"
      );
    }
    if (!plan.imagesPayload) setModule(modules, "IMAGES", "NOT_REQUESTED");
    return {
      operation: "FULL_PRODUCT_SYNC",
      productId: input.productId,
  ...
  async execute(input: {
    ...
    if (reservation.state === "IN_FLIGHT") {
      return {
        ...
        planFingerprint: plan.planFingerprint,
        protectedFingerprintBefore: fingerprintBlingFullProductValue(protectedRemoteSnapshot(remote, plan)),
    ...
    const protectedFingerprintBefore = fingerprintBlingFullProductValue(protectedRemoteSnapshot(remote, plan));
    ...
    try {
      if (Object.keys(plan.mainPayload).length) {
        ...
        setModule(modules, "PRODUCT_FIELDS", "NOT_REQUESTED");
      }

      if (!plan.priceCostPayload) {
        setModule(modules, "PRICE_COST", "NOT_REQUESTED");
      ...
      if (plan.stockPayload) {
        ...
        setModule(modules, "STOCK", "NOT_REQUESTED");
      ...
      if (plan.imagesPayload) {
        ...
        setModule(modules, "IMAGES", "NOT_REQUESTED");
      }
