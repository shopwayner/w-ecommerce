...
import { Bell, LogOut, Menu, Moon, Plus, Sun, UserRound, X } from "lucide-react";
...
function formatSyncChange(change: SyncReportEntry) {
  ...
  if (["ATTRIBUTES", "OTHER"].includes(change.category)) {
    return change.field;
  }
  ...
  useEffect(() => {
    ...
    async function refreshUnreadCount() {
      try {
        ...
        // The full notification panel remains the user-visible error surface.
      }
    }
    ...
    window.addEventListener("w-notifications-updated", handleRefresh);
    return () => {
      active = false;
      window.clearInterval(interval);
      window.removeEventListener("focus", handleRefresh);
      window.removeEventListener("w-notifications-updated", handleRefresh);
    };
  }, []);
  ...
  return (
    ...
    >
      <div className={cn("flex h-16 items-center gap-2 px-3 py-2 sm:px-4 lg:px-5", denseDesktopShell && "lg:!h-[3.75rem] lg:!gap-[7px] lg:!px-1")}>
                      ...
                      >
                        ...
                        {notification.action ? (
                          <>
                            <div className="mt-2 space-y-2">
                              {notification.action.preview.groups.map((group) => (
                                <div key={group.category}>
                                  <p className="text-[11px] font-semibold text-matrix-fg">
                                    {blingProductSyncCategoryLabels[group.category]} - {group.total}
                                  </p>
                                  ...
                                  {group.total > group.items.length ? (
                                    <p className="mt-1 text-[10px] text-matrix-muted">
                                      Mostrando {group.items.length} de {group.total} alteracoes.
                                    </p>
                                  ) : null}
                                </div>
                              ))}
                            ...
                            <button
                              className="mt-3 text-xs font-semibold text-matrix-gold hover:text-matrix-goldDark"
                              onClick={() => {
        ...
        {selectedSyncJobId ? (
          <div
            className="fixed inset-0 z-[100] grid place-items-center bg-black/75 px-3 py-5 backdrop-blur-sm"
            onClick={() => setSelectedSyncJobId(null)}
          >
            <section
              aria-label="Relatorio da sincronizacao Bling"
              aria-modal="true"
              className="matrix-scroll max-h-[90vh] w-full max-w-3xl min-w-0 overflow-y-auto rounded-lg border border-matrix-gold/35 bg-matrix-panel p-4 sm:p-5"
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-matrix-muted">
                    Somente produtos que tiveram dados atualizados.
                  </p>
                </div>
                <button
                  aria-label="Fechar relatorio"
                  className="grid h-9 w-9 shrink-0 place-items-center rounded-md border border-matrix-border text-matrix-muted hover:text-matrix-gold"
                  onClick={() => setSelectedSyncJobId(null)}
                  type="button"
                >
                  <X className="h-4 w-4" />
              ...
              <div className="mt-4 flex min-w-0 flex-wrap items-center gap-2">
                <label className="text-xs font-semibold text-matrix-muted" htmlFor="sync-report-category">
                  Categoria
                </label>
                <select
                  className="min-w-0 rounded-md border border-matrix-border bg-matrix-panel2 px-3 py-2 text-xs text-matrix-fg"
                  id="sync-report-category"
                  onChange={(event) => {
                ...
                >
                  ...
                  {blingProductSyncCategories.map((category) => (
                    <option key={category} value={category}>
                      {blingProductSyncCategoryLabels[category]}
                    </option>
                  ))}
              ...
              <div className="mt-5 space-y-3">
                {syncReportLoading ? (
                  <p className="rounded-md border border-matrix-border bg-matrix-panel2 p-3 text-sm text-matrix-muted">
                    Carregando relatorio...
                  </p>
                ) : null}
                {!syncReportLoading && syncReportError ? (
                  <p className="rounded-md border border-red-500/30 bg-red-950/20 p-3 text-sm text-red-200">
                    {syncReportError}
                  </p>
                ) : null}
                {!syncReportLoading && !syncReportError && syncReportData ? (
                  <>
                    <p className="text-xs text-matrix-muted">
                      {syncReportData.summary.changedProducts} produtos alterados; {syncReportData.summary.totalChanges} alteracoes; {syncReportData.summary.failureCount} falhas.
                    </p>
                    <div className="divide-y divide-matrix-border rounded-md border border-matrix-border bg-matrix-panel2/60">
                      ...
                      {syncReportData.total === 0 ? (
                        <p className="p-3 text-sm text-matrix-muted">Nenhum item nesta categoria.</p>
                      ) : null}
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <button
                        className="rounded-md border border-matrix-border px-3 py-2 text-xs font-semibold text-matrix-fg disabled:opacity-40"
                        disabled={syncReportData.page <= 1}
                        onClick={() => setSyncReportPage((current) => Math.max(1, current - 1))}
                        type="button"
                      >
                        Anterior
                      </button>
                      <span className="text-xs text-matrix-muted">
                        Pagina {syncReportData.page} de {syncReportData.totalPages} - {syncReportData.total} itens
                      </span>
                      <button
                        className="rounded-md border border-matrix-border px-3 py-2 text-xs font-semibold text-matrix-fg disabled:opacity-40"
                        disabled={syncReportData.page >= syncReportData.totalPages}
                        onClick={() => setSyncReportPage((current) => current + 1)}
                        type="button"
                      >
                        Proxima
                      </button>
